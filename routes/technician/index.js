...
router.post('/order/status/update', async ctx => {
	try {
		// the logic goes here
		const tech_username = ctx.session.username;
		const f = ctx.request.body;
		const mOrders = await new Orders(dbName);

		if (f.order0) {
			// update first record
			mOrders.updateOrderStatus(tech_username, f.order0, f.status0);
		}
		if (f.order1) {
			// update second record
			mOrders.updateOrderStatus(tech_username, f.order1, f.status1);
		}
		if (f.order2) {
			// update thrird record
			mOrders.updateOrderStatus(tech_username, f.order2, f.status2);
		}
