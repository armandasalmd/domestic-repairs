...
module.exports = class Job {
    ...
    constructor(dbJob = ':memory:') {
        return(async () => {
			...
			const sql = 'CREATE TABLE IF NOT EXISTS jobs (jobid INTEGER PRIMARY KEY AUTOINCREMENT, manufacturer TEXT, type TEXT, age INT, issue TEXT, status TEXT, techid INT, datetimefrom DATETIME, datetimeto DATETIME);';
    ...
    async submitJob(manufacturer, type, age, issue) {
        try {
			if (manufacturer.length === 0) throw new Error('Choose appliance manufacturer');
			...
			sql = `INSERT INTO jobs(manufacturer, type, age, issue, status) VALUES("${manufacturer}", "${type}", "${age}", "${issue}", "Submitted")`;
			await this.db.run(sql);
			return true;
		} catch (err) {
			throw err;
    ...
    async processJob(technicianID, jobID, datefrom, dateto) {
        try {
            ...
            let sql = `SELECT COUNT(techid) as techrecord FROM techs WHERE techid="${technicianID}";`;
            const data = await this.db.get(sql);
            if (data.techrecord == 0)
                throw new Error(`Technician with ID "${technicianID}" does not exist`);
